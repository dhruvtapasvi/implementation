...
def meanSquaredErrorLossConstructor(inputRepresentationDimensions, decodedInputRepresentationVariance=None):
    ...
    def meanSquaredErrorLoss(inputRepresentation, decodedInputRepresentation):
        if decodedInputRepresentationVariance is not None:
            lossPerPixel = 0.5 * math.log(2 * math.pi) + 0.5 * log(decodedInputRepresentationVariance + epsilon) + 0.5 * square(decodedInputRepresentation - inputRepresentation) / (decodedInputRepresentationVariance + epsilon)
            ...
            return mean(totalNumberOfPixels * 0.5 * (
                math.log(2 * math.pi) + mean_squared_error(flatten(inputRepresentation), flatten(decodedInputRepresentation))
            ))
